...
app.layout = html.Div([
    ...
    html.Div(id='output-message', style={'textAlign': 'center', 'color': '#FFFFFF', 'fontFamily': 'Roboto', 'fontSize': '27px', 'marginTop': '20px'})
...
def plot_time_signature(df_weekday, df_weekday_arrival, focus_zone):
    ...
    fig.update_layout(
        title={
            'text': f'Percentage of Arrivals to and Departures from Zone {focus_zone} Over the Day',
            'y': 0.95,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
            'font': {'size': 24}  # Increased title font size
        },
        xaxis_title={'text': 'Hour of Day', 'font': {'size': 21}},  # Increased axis title font size
        yaxis_title={'text': 'Percentage of Trips', 'font': {'size': 21}},
        barmode='group',
        legend=dict(x=0.7, y=1, bgcolor='rgba(0, 0, 0, 0.5)', font={'size': 18}),  # Increased legend font size
        paper_bgcolor='#1E1E1E',
        plot_bgcolor='#1E1E1E',
        font=dict(color='#FFFFFF', size=21, family='Roboto'),  # Increased general font size
        height=500
    )

    fig.update_xaxes(tickfont=dict(size=18))  # Increased tick font size
...
def plot_trips_by_distance(focus_zone):
    ...
    fig.update_layout(
        title={
            'text': f'Histogram of Trips to Zone {focus_zone} by Distance',
            'y': 0.95,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
        ...
        xaxis_title={'text': 'Distance (km)', 'font': {'size': 21}},  # Increased axis title font size
        yaxis_title={'text': 'Number of Trips', 'font': {'size': 21}},
        legend=dict(x=0.7, y=1, bgcolor='rgba(0, 0, 0, 0.5)', font={'size': 18}),  # Increased legend font size
        paper_bgcolor='#1E1E1E',
        plot_bgcolor='#1E1E1E',
        font=dict(color='#FFFFFF', size=21, family='Roboto'),  # Increased general font size
...
def create_geopandas_map(focus_zone):
    ...
    colorscale = [
        ...
        [0.1, 'rgba(173,216,230,0.6)'],  # Light blue
        [0.25, 'rgba(135,206,235,0.6)'],
        [0.5, 'rgba(65,105,225,0.6)'],
        [0.75, 'rgba(0,0,255,0.6)'],
        [1, 'rgba(25,25,112,0.6)']  # Dark blue
    ]
    ...
    # Update layout for dark theme and improved readability with increased font sizes
    fig.update_layout(
        mapbox_style="carto-darkmatter",  # Dark base map with white lines
        mapbox=dict(
        ...
        height=700,
        title={
            'text': f'Trips per 10,000 people to Zone {focus_zone} (Shown in Red)',
            'y': 0.98,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
